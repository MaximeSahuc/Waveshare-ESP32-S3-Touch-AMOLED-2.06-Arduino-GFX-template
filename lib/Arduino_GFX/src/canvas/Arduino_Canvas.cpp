...
#if defined(ESP32)
    // LOCAL PATCH (project copy only): a full-screen framebuffer for the 410x502
    // CO5300 AMOLED is ~402 KB and does not fit the ~320 KB internal RAM. When
    // PSRAM is present, allocate the framebuffer there (8 MB OPI PSRAM on this
    // board); fall back to internal RAM otherwise. Upstream uses aligned_alloc
    // (internal-only), which would fail begin() on this panel.
    if (psramFound())
    {
      _framebuffer = (uint16_t *)heap_caps_aligned_alloc(16, s, MALLOC_CAP_SPIRAM);
    }
    if (!_framebuffer)
    {
